...
class spp_container( Layer ):
    def __init__( self, incoming, **kwargs ):
        ...
        self.spp4 = SPP_cpu( incoming, image_x=9,  image_y=14, name='spp4' )
        self.spp5 = SPP_cpu( incoming, image_x=14, image_y=9,  name='spp5' )

    def get_output_for( self, input, **kwargs ):
        dim_x = T.shape( input )[3]
        dim_y = T.shape( input )[2]

        dim_x = Print( 'dim_x', dim_x )
        dim_y = Print( 'dim_y', dim_y )

        aspect = T.cast( dim_y, 'float32' ) / dim_x

        border_07 = np.array( [0.7] )
        border_09 = np.array( [0.9] )
        border_11 = np.array( [1.1] )
        border_13 = np.array( [1.3] )
        ...
        branches = ifelse( T.gt(aspect, border_13)[0],
                      ...
                      ifelse( T.gt( aspect, border_11 )[0],
                         ...
                         ifelse( T.lt( aspect, border_07)[0],
                             self.spp5.get_output_for( input, **kwargs ),
                             ifelse( T.lt( aspect, border_09)[0],
